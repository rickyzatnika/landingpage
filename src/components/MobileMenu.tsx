  ...
  return (
    <div className="block md:hidden ">
      <div className="flex flex-col items-center gap-[4.5px] cursor-pointer " onClick={() => setIsOpen(prev=>!prev)}>
          <div className={`w-6 h-1 bg-black rounded-sm origin-left ${isOpen ? "rotate-45 transition-all ease-linear duration-200" : ""} `}/>
          <div className={`w-6 h-1 bg-black rounded-sm ${isOpen ? "opacity-0 transition-all ease-linear duration-200" : ""}`}/>
          <div className={`w-6 h-1 bg-black rounded-sm origin-left ${isOpen ? "-rotate-45 transition-all ease-linear duration-200" : ""} `}/>
      </div>
